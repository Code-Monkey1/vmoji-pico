...
module motor_collet(
    ...
    assert(clamp_offset + screw_d / 2 < r, "clamp_offset outside collet");
    assert(clamp_offset - screw_d / 2 > bore_d / 2 + min_bore_to_clamp,
        "clamp_offset too close to motor bore");
    assert(collet_flange_od > od, "flange OD must exceed barrel OD");
    assert(flange_bolt_r + mount_hole_d / 2 < collet_flange_od / 2 - 1,
        "flange bolts too close to rim");
    assert(flange_bolt_r - mount_hole_d / 2 > od / 2 + 0.5,
        "flange bolts collide with barrel");

    diff() {
        union() {
            cyl(d = od, h = motor_h, anchor = BOTTOM);
            mb_flange_solid(collet_flange_od, collet_flange_h, key_w, key_d);
        }
        ...
        tag("remove") {
            ...
            up(wire_slot_h / 2 - eps / 2)
                right(bore_d / 4)
                    cuboid(
                        [od / 2 + key_d + 2, wire_slot_w, wire_slot_h + eps],
                        anchor = LEFT + CENTER
                    );
